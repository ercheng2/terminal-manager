...
class RemoteDesktopViewer:
        ...
        while self.running:
            ...
            try:
                ...
                while self.running:
                    # 新帧协议：21字节帧头 !BIIIII
                    frame_header = b''
                    while len(frame_header) < 21 and self.running:
                        try:
                            chunk = sock.recv(21 - len(frame_header))
                            if not chunk:
                    ...
                    if not self.running:
                        break
                    if len(frame_header) < 21:
                        raise ConnectionError("Incomplete frame header")
                    
                    frame_type, rx, ry, rw, rh, jpeg_len = struct.unpack('!BIIIII', frame_header)
                    ...
                    # 接收JPEG数据
                    jpeg_data = bytearray()
                    while len(jpeg_data) < jpeg_len and self.running:
                        try:
                            chunk = sock.recv(min(jpeg_len - len(jpeg_data), 262144))
                            if not chunk:
                                raise ConnectionError()
                    ...
                    self._raw_frame = (frame_type, rx, ry, rw, rh, jpeg_data)
    ...
    def _decode_worker(self):
        ...
        self._current_screen_img = None  # 增量帧基础图像
        while self.running:
    ...
    def _sync_remote_clipboard_to_local(self):
        """Ctrl+C后：从远程拉剪贴板内容到本地"""
        try:
    ...
    def _push_local_clipboard_to_remote(self):
        """Ctrl+V时：同步推送本地剪贴板到远程，确保远程剪贴板就绪后再发Ctrl+V"""
        import http.client
        try:
            clip_data = self._read_local_clipboard()
            if clip_data:
                payload = {"input_type": "set_clipboard"}
                payload.update(clip_data)
                conn = http.client.HTTPConnection(self.client_ip, 5901, timeout=5)
                ...
                try:
                    ...
                    # 非小键盘按键
                        ...
                        if mods:
                            ...
                            # Ctrl+C: 远程复制后，等200ms把远程剪贴板拉回本地
                            if 'ctrl' in mods and ch in ('c', 'C'):
                                try:
                                    self.win.after(200, self._sync_remote_clipboard_to_local)
                                except:
                                    pass
                            # Ctrl+V: 先把本地剪贴板推到远程，再让远程粘贴
                            if 'ctrl' in mods and ch in ('v', 'V'):
                                try:
                                    self._push_local_clipboard_to_remote()
                                except:
                                    pass
                        else:
